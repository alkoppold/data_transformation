...
``` {r load-prepare-data}
data_multiverse = read_rds("data/Merz_multiverse.rds")
...
data_multi_rel[order(data_multi_rel$rel),'approach_eff_order'] <-  1:nrow(data_multi_rel)
...
p_reliability <- data_multi_rel %>%
  ggplot(aes(x = approach_eff_order, y = rel, color = transformation)) +
  geom_point() +
...
# plot specifications of parameters
s_reliability <- ggplot(data_multi_rel, aes(x = approach_eff_order, y = range_cor)) +
  geom_point(shape = '|', size = size_point) +
   #scale_color_manual(values = rep('black', length(levels(res_ttest_d$approach)) )) +
  scale_y_discrete(name = 'Range correction', labels = c("none", "CS corrected", "US corrected")) +
    scale_x_continuous(breaks = c(1:15), labels = c(1:15)) +
