...
data_long = data_long %>% rename(id = Versuchspersonennummer, scr_raw = scr) %>% 
  select(id, trialnr, stimulus, scr_raw) %>% #get rid of "condition" and reorder
...
data_long = data_long %>% 
  ...
  mutate(scr_box = (scr_shift ^ lambda - 1) / lambda)

#View(data_long)
#data_long %>% ggplot(aes(x = scr_raw, y = scr_box, color = stimulus)) + geom_point() + theme_bw()

data_long = data_long %>% select(-scr_shift, -lambda) #remove helping variables
...
#TODO doesn't work like this. probably need wider data format
# data_long = data_long %>% group_by(id) %>% 
#   mutate(scr_max_cs = data_long %>% filter(stimulus %>% grepl("^cs", .)) %>% pull(scr_raw) %>% max(),
#          scr_max_us = data_long %>% filter(stimulus %>% grepl("ucs", .)) %>% pull(scr_raw) %>% max())
...
data_line$stimulus <- factor(data_long$stimulus, levels = c("csd","cspe","csm","ucs"))
...
#TODO: combine some conditions? (e.g., cspu + cspe; ucsm + ucsw; cf. SCR mean plots)
