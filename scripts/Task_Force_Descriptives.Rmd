...
data_plot %>% 
  ...
  facet_grid(range_cor + stimulus ~ transformation, labeller = "label_both", scales = "free_x") + 
...
data_plot %>% 
  ...
  facet_grid(stimulus + range_cor ~ transformation, labeller = "label_both", scales = "free_x") + 
  geom_histogram(color="black") + 
...
data_plot %>% filter(stimulus == "csd") %>% 
  ...
  ggplot(aes(x = scr_scaled, fill = range_cor)) +
  facet_grid(range_cor ~ transformation, labeller = "label_both", scales = "free_x") + 
  geom_histogram(color="black") + 
  myGgTheme + scale_fill_viridis_d(option="C")
```
...
effects = data_long %>% filter(stimulus=="csd") %>% 
  ...
  summarise(d = mean(scr)/sd(scr), #checked against apa::cohens_d(scr)
  ...
  group_by(transformation) %>% mutate(rank_rc = rank(-d)) %>% ungroup() %>% 
  group_by(range_cor) %>% mutate(rank_trans = rank(-d)) %>% ungroup() %>% 
  arrange(desc(d))
