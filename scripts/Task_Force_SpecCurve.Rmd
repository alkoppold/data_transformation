...
# Box cox transform the data -> Hier stimmt was noch nicht! R findet negative Werte im Datensatz, sind aber alle >=0
# Aber darum: 0.0000000000000000001+
b <- boxcox(lm(0.0000000000000000001+data_long$scr_raw ~ 1))
# Extract lambda
...
### Add columns for transformation (log, sqrt, box cox) and range correction type (none, CS corrected, US corrected)
...
p1 <- res_ttest_d %>%
  ...
  scale_color_manual(name = 'Transformation', values = clrs[c(1:4)], labels = c("box cox", "log-transformed", "none", "square-root")) +
