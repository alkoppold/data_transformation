...
effects = data_long %>% filter(stimulus=="csd") %>% 
  ...
  summarise(d = mean(scr)/sd(scr), #checked against apa::cohens_d(scr) #TODO use Hedge's g instead
...
``` {r export data}
data_long %>% write_rds("Merz_data.rds" %>% paste0("data/", .))
reliabilities %>% write_rds("Merz_reliability.rds" %>% paste0("data/", .))
```
