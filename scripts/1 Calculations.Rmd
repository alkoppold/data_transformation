...
# Choose the dataset
data_choice <- readline(prompt = "Enter the number of the dataset you want to use (1 = merz eda, 2 = klingelh eda, 3 = reutter hr): ") 
data_of_interest <- switch(data_choice,
                            "1" = "merz_eda", "2" = "klingelh_eda",
                            "3" = "reutter_hr",
                            "")

# Load the selected dataset
if (data_of_interest == "merz_eda") {
...
} else if (data_of_interest == "klingelh_eda") {
  ...
  data_multiverse <- readRDS("data/heart.rds") %>%
    ...
    rename(stimulus = threat,
           id = subject) %>%
    mutate(stim_cat = if_else(stimulus == 6, "cs", "ucs"),
...
data_long %>% write_rds("Reutter_transformations.rds" %>% paste0("data/", .))
```
...
effects = data_long %>% filter(stimulus=="csd") %>% 
  ...
  # TODO remove effect column
  dplyr::summarise(effect = mean(scr)/sd(scr), #checked against apa::cohens_d(scr) #TODO use Hedge's g instead
            hedges_g_apa = apa::cohens_d(scr, corr = "hedges_g"), #hedges g calculated with the apa package
  ...
  mutate(hedges_g_manual = effect * (1 - (3 / (4 * 95 - 1) ))) %>% # manually added hedges g from Cumming,2012
...
data_multiverse %>% write_rds("Reutter_multiverse.rds" %>% paste0("data/", .))
