...
data_long.max %>% filter(max_cs == 0) # run on pupil data -- they do not have US stimuli
...
data_long = data_long %>% full_join(data_long.max) %>% 
  mutate(scr_rc_cs = scr / max_cs
         #scr_rc_us = if ("max_ucs" %in% names(.)) scr / max_ucs else scr) %>% # new by alina, for hr // Do not run this line for pupil data since they do not have USs
         ) %>% 
...
reliabilities.permutations = tibble(
  ...
  # Added by Alex: For pupil data we exclude the following rows, since the there is no US inn the pupil data
...
effects = na.omit(data_long) %>% filter(stimulus=="csd") %>%  # Added by Alex: na.omit needs to be wrapped around "data_long" otherwise NA value are created when calculating the cohen_d and t_tests.
