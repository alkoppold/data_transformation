...
dataset_1 = res_ttest_g
dataset_1$reinforcement_rate = 
dataset_1$n = 100
dataset_1$CS_duration = "6s" 
dataset_1$CS_stimuli = "picture"
dataset_1$training = 
dataset_1$max_ampl = 
...
# Load data
load(here("data/dataSCR_B07.RData"))
...
### Add different transformations
# Log-transform the raw data
dataSCR$scr_log <- log10(1+dataSCR$scr_raw)

# Square root transform the raw data
dataSCR$scr_sqr <- sqrt(dataSCR$scr_raw)

# Box cox transform the data: the added positive constant is the smallest raw SCR wihtin the data set
b <- boxcox(lm(0.01007+dataSCR$scr_raw ~ 1))
# Extract lambda
lambda <- b$x[which.max(b$y)]
lambda
dataSCR$scr_box <- (dataSCR$scr_raw ^ lambda - 1)/lambda

# z transform the data
dataSCR$scr_ztr <- scale(dataSCR$scr_raw)


### Prepare range correction by calculating max SCR of CS and US responses
# Max amplitude CS
dataSCR$scr_max_cs <- NA
for (i in unique(dataSCR$id)) {
  data_select <- subset(dataSCR, id == i & cs == "CS_P" | id == i & cs == "CS_M")
  max_ampl_cs <- max(data_select$scr_raw, na.rm = T)
  dataSCR$scr_max_cs[which(dataSCR$id == i)] <- max_ampl_cs
}


# Max amplitude US
dataSCR$scr_max_us <- NA
for (i in unique(dataSCR$id)) {
  data_select <- dataSCR[which(dataSCR$id == i & grep("US", dataSCR$cs)), ]
  max_ampl_us <- max(data_select$scr_raw, na.rm = T)
  dataSCR$scr_max_us[which(dataSCR$id == i)] <- max_ampl_us
}


### Range correct all transformation types with CS-rc
dataSCR$scr_raw_rc_cs <- dataSCR$scr_raw/dataSCR$scr_max_cs
dataSCR$scr_log_rc_cs <- dataSCR$scr_log/dataSCR$scr_max_cs
dataSCR$scr_sqr_rc_cs <- dataSCR$scr_sqr/dataSCR$scr_max_cs
dataSCR$scr_box_rc_cs <- dataSCR$scr_box/dataSCR$scr_max_cs
dataSCR$scr_ztr_rc_cs <- dataSCR$scr_ztr/dataSCR$scr_max_cs

### Range correct all transformation types with US-rc
dataSCR$scr_raw_rc_us <- dataSCR$scr_raw/dataSCR$scr_max_us
dataSCR$scr_log_rc_us <- dataSCR$scr_log/dataSCR$scr_max_us
dataSCR$scr_sqr_rc_us <- dataSCR$scr_sqr/dataSCR$scr_max_us
dataSCR$scr_box_rc_us <- dataSCR$scr_box/dataSCR$scr_max_us
dataSCR$scr_ztr_rc_us <- dataSCR$scr_ztr/dataSCR$scr_max_us
...
data <- gather(dataSCR, key = "approach", value = "scr_mean", - c(id, cs, cs_time))
data <- data %>% filter(approach != "scr_max_cs") # this is not a transformation procedure 
data <- data %>% filter(approach != "scr_max_us")
...
# variables of interest
dataset_2 = res_ttest_d  # set B07 dataset here
dataset_2$reinforcement_rate = 
dataset_2$n = 100
dataset_2$CS_duration = "6s" 
dataset_2$CS_stimuli = "picture"
dataset_2$training = 
dataset_2$max_ampl = 
