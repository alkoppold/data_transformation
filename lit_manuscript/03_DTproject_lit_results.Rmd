...
```{r results-load-data}
library(tidyverse)
data_extract = read_rds("data/data_extract.rds")
```
...
#data.frame(colname = data_extract %>% colnames()) %>% mutate(colnum = 1:n()) %>% select(colnum, everything())
df <- data_extract %>% select(37:44)
n_total <- nrow(df)
