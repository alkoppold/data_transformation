...
# Example: assume df_check is your data.frame with columns named exactly as you described:
# For demonstration, let's create a fake sample with similar structure

df_check = data_extract %>% select(20:32)

names(df_check) <- c(
  "normal_checked",        # "Was the normal distribution checked? answers: dependent variable, independent variable, residuals, mixed, not reported"
  "normal_how",            # "If yes, how? (e.g., specific test or visually), if no: NA"
  "normal_before_after",   # "If yes, was the normal distribution tested before or after transformation procedure? answers: before, after, both, not reported. if no: NA"
  "homoscedasticity_checked",  # "Was the homoscedasticity checked? answers: yes, not reported"
  "homoscedasticity_how",       # "If yes, how?  (e.g., specific test or visually), if no: NA...24"
  "sphericity_checked",          # "Was the sphericity checked? answers: yes, not reported"
  "sphericity_how",               # "If yes, how?  (e.g., specific test or visually), if no: NA...26"
  "independence_checked",         # "Was the independence of residuals checked? answers: yes, not reported"
  "independence_how",             # "If yes, how?  (e.g., specific test or visually), if no: NA...28"
  "linearity_checked",            # "Was the linearity checked? answers: yes, not reported"
  "linearity_how",                # "If yes, how?  (e.g., specific test or visually), if no: NA...30"
  "multicollinearity_checked",    # "Was the multicollinearity checked? answers: yes, not reported"
  "multicollinearity_how"         # "If yes, how?   (e.g., specific test or visually), if no: NA"
)


# Map the questions to their corresponding "how" columns and a short tag
library(tidyverse)

# Assuming df_check is your data with renamed columns, and categories defined as:
categories <- tibble(
  main_q = c(
    "normal_checked",
    "homoscedasticity_checked",
    "sphericity_checked",
    "independence_checked",
    "linearity_checked",
    "multicollinearity_checked"
  ),
  how_q = c(
    "normal_how",
  ...
  tag = c("Normality", "Homoscedasticity", "Sphericity", "Independence", "Linearity", "Multicollinearity")
)

# Use map2_dfr to create long data for plotting
plot_data <- purrr::map2_dfr(categories$main_q, categories$how_q, function(main_col, how_col) {
  tag_name <- categories$tag[categories$main_q == main_col]
  
  main_counts <- df_check %>%
    count(response = .data[[main_col]]) %>%
    mutate(type = "Main", question = tag_name)
  
  how_counts <- df_check %>%
    filter(!is.na(.data[[main_col]])) %>%
    count(response = .data[[how_col]]) %>%
    mutate(type = "If How", question = tag_name)
  
  bind_rows(main_counts, how_counts)
}) %>%
  filter(!is.na(response), response != "") %>%
  mutate(
    response = fct_reorder(response, n),
    type = factor(type, levels = c("Main", "If How")),
    question = factor(question, levels = categories$tag)
  )

# Plot
ggplot(plot_data, aes(x = fct_rev(response), y = n, fill = type)) +
  geom_col(position = position_dodge(width = 0.7), width = 0.7) +
  facet_wrap(~question, scales = "free_y", ncol = 2) +
  coord_flip() +
  scale_fill_manual(values = c("Main" = "#4477AA", "If How" = "#EE6677")) +
  ...
  labs(
    title = "Reporting of Assumption Checks and Methods",
    subtitle = "Main answers vs. how the check was conducted",
    caption = "Across different statistical assumptions"
  )
...
library(scales)
library(glue)
library(viridis) # modern color palette

# Filter for "Repeated Measures ANOVA" articles
df_rmanova <- data_extract %>%
  ...
  select(sphericity_checked = 27, sphericity_how = 26) %>%
...
plot_data <- bind_rows(
  df_rmanova %>%
    ...
    mutate(type = "Was Sphericity Checked?"),
  
  df_rmanova %>%
    filter(!is.na(sphericity_how) & sphericity_how != "") %>%
    count(response = sphericity_how) %>%
    mutate(type = "How Was It Checked?")
) %>%
  filter(!is.na(response), response != "") %>%
  mutate(
    percent = n / n_articles_rmANOVA,
    response = fct_reorder(response, n),
    type = factor(type, levels = c("Was Sphericity Checked?", "How Was It Checked?"))
  )

# Plot with modern style
ggplot(plot_data, aes(x = fct_rev(response), y = percent, fill = type)) +
  geom_col(position = position_dodge(width = 0.7), width = 0.6, alpha = 0.9) +
  geom_text(aes(label = paste0(n, " (", percent(percent, accuracy = 1), ")")),
            position = position_dodge(width = 0.7), hjust = -0.05, size = 4, color = "gray20") +
  coord_flip() +
  ...
  labs(
    title = "Sphericity Assumption in Repeated Measures ANOVA",
    subtitle = glue("{n_articles_rmANOVA}/{n_total} articles using RM-ANOVA")
   # caption = "Includes whether sphericity was assessed and, if so, how it was checked"
  )
