...
names(df_cake) <- c("Normality","Homoscedasticity","Sphericity","Linearity","Mulitcollinearity")
...
```{r assumptions-homosc-linearity-mulitcoll}
...
# Patchwork the pies
coll_pies <- pies[[1]] + pies[[2]] + pies[[4]] 
...
# Plot it
ggplot(df_long_stats, aes(ymax = ymax, ymin = ymin, xmax = 1, xmin = 0, fill = Model)) +
  geom_rect(color = "white") +
  # Draw horizontal segments connecting the slice edge (x=1) to near label (x=1.1)
  geom_segment(
    aes(x = 1, xend = 1.1, y = label_pos, yend = label_pos),
    color = "grey50"
  ) +
  # Use geom_text_repel for labels, no segments this time
  geom_text_repel(
    aes(x = 1.2, y = label_pos, label = label),
    size = 4, fontface = "bold", color = "black",
    segment.color = NA,   # disables ggrepel's default segment lines
    nudge_x = 0,
    direction = "y",
    hjust = 0
  ) +
  coord_polar(theta = "y") +
  xlim(c(-0.5, 1.5)) +  # room for labels on the right
  scale_fill_scico_d(palette = "hawaii", direction = -1) +
  theme_void(base_family = "Calibri") +
  theme(
    legend.position = "right",
