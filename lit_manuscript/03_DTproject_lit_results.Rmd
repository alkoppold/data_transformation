...
# Customize model column (statistical): split several entries in one cell


# Set names for plot titles
names(df_cake) <- c("Normality","Homoscedasticity","Sphericity","Linearity","Mulitcollinearity","Models")
...
normality_plot <- pies[[5]] + plot_normal_bar +
  plot_annotation(
...
plot_sphericity_bar <- ggplot(df_bar, aes(x = response, y = n, fill = response)) +
  geom_col(width = 0.7) +
  geom_text(
...
# Delete the title of the pie chart
pies[[6]] <- pies[[6]] + theme(plot.title = element_blank())

# Combine the pie chart with the bar chart
sphericity_plot <- pies[[6]] + plot_sphericity_bar +
  plot_annotation(
