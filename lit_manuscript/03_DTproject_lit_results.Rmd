...
# Load data
data_extract <- readr::read_rds(
  here("data", "data_extract.rds")
) #cf. script "extract_cleanup.R"

```


```{r results-define-plot-theme}
...
df <- data_extract %>% select(doi, `HR`:pupil)
...
df_cake <- df_check %>% select(normality, homoscedasticity, sphericity_how, linearity, multicollinearity)

# Customize normality column: if normality is not "not reported" -> yes
df_cake$normality[df_cake$normality != "not reported"] <- "yes"
...
plot_normal_bar <- df_check %>%
  ...
  ggplot(aes(x = fct_reorder(normality, n), y = n, fill = normality)) +
...
data_trans_scr <- data_trans_scr %>%
  make_long(
    cols = c(scr_transf, transf_level, rationale_di)
  )
...
data_trans_scl <- data_trans_scl %>%
  make_long(
    cols = c(scl_transf, transf_level, rationale_di)
  )
...
data_outlier <- data_extract %>%
  select(outlier, outlier_when, outlier_how) %>%
...
#TODO: take the long format, delete non-numeric values?

violin_n <- ggplot(aes(x = 1, y = as.numeric(n_after_exclusion)), data = data_extract) +
  ...
  xlab("") +
  ylab("Number of participants after exclusion") +
...
# #TODO: Do we want to use the column?
# 
